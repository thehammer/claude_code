...
State lives at `~/.claude/queue/` (plain JSON, gitignored). See
...
5. **On explicit approval, enqueue.** Save the plan to a temp file and call
   `queue add`. Never enqueue without the user saying go.
...
In phase 2 once the daemon is live, step 3 goes away — `queue add` is enough,
