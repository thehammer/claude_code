...
const transport_util_js_1 = require("./transport.util.js");
...
describe('Transport Utility', () => {
    ...
    describe('getAtlassianCredentials', () => {
        ...
        it('should return null when environment variables are missing', () => {
            ...
            const origSiteName = config_util_js_1.config.get('ATLASSIAN_SITE_NAME');
            const origUserEmail = config_util_js_1.config.get('ATLASSIAN_USER_EMAIL');
            const origApiToken = config_util_js_1.config.get('ATLASSIAN_API_TOKEN');
            ...
            try {
                ...
                config_util_js_1.config.get = (key) => testConfig[key];
                // Call the function
                const credentials = (0, transport_util_js_1.getAtlassianCredentials)();
                ...
                config_util_js_1.config.get = (key) => {
                    if (key === 'ATLASSIAN_SITE_NAME')
                        return origSiteName;
                    if (key === 'ATLASSIAN_USER_EMAIL')
                        return origUserEmail;
                    if (key === 'ATLASSIAN_API_TOKEN')
                        return origApiToken;
                    return config_util_js_1.config.get(key);
                };
            }
        });
    });
    describe('fetchAtlassian', () => {
        it('should successfully fetch data from the Atlassian API', async () => {
            ...
            const result = await (0, transport_util_js_1.fetchAtlassian)(credentials, '/rest/api/3/project/search', {
                method: 'GET',
                headers: {
        ...
        it('should handle API errors correctly', async () => {
            ...
            await expect((0, transport_util_js_1.fetchAtlassian)(credentials, '/rest/api/3/non-existent-endpoint')).rejects.toThrow();
        }, 15000); // Increased timeout for real API call
        ...
        it('should support custom request options', async () => {
            ...
            const result = await (0, transport_util_js_1.fetchAtlassian)(credentials, '/rest/api/3/project/search?maxResults=1', options);
