...
const atlassian_comments_types_js_1 = require("./atlassian.comments.types.js");
const atlassian_comments_controller_js_1 = __importDefault(require("../controllers/atlassian.comments.controller.js"));
// Create a contextualized logger for this file
const toolLogger = logger_util_js_1.Logger.forContext('tools/atlassian.comments.tool.ts');
...
async function listComments(args) {
    const methodLogger = logger_util_js_1.Logger.forContext('tools/atlassian.comments.tool.ts', 'listComments');
    methodLogger.debug('Listing Jira comments with args:', args);
    try {
        const result = await atlassian_comments_controller_js_1.default.listComments(args);
...
async function addComment(args) {
    const methodLogger = logger_util_js_1.Logger.forContext('tools/atlassian.comments.tool.ts', 'addComment');
    ...
    try {
        const result = await atlassian_comments_controller_js_1.default.addComment(args);
...
function registerTools(server) {
    const methodLogger = logger_util_js_1.Logger.forContext('tools/atlassian.comments.tool.ts', 'registerTools');
    methodLogger.debug('Registering Atlassian Comments tools...');
    // Register the list comments tool
    server.tool('jira_ls_comments', `Lists comments for a specific Jira issue identified by \`issueIdOrKey\`. Supports pagination via \`limit\` and \`startAt\`. Returns a formatted Markdown list of comments with author, date, and content. Pagination information including next page instructions is included in the returned text. Requires Jira credentials to be configured.`, atlassian_comments_types_js_1.ListCommentsToolArgs.shape, listComments);
    // Register the add comment tool
    server.tool('jira_add_comment', `Adds a new comment to a specific Jira issue identified by \`issueIdOrKey\`. The content of the comment is provided in \`commentBody\` and supports Markdown formatting which will be converted to Jira's Atlassian Document Format (ADF). Returns a confirmation message and link to the newly created comment. Requires Jira credentials to be configured.`, atlassian_comments_types_js_1.AddCommentToolArgs.shape, addComment);
