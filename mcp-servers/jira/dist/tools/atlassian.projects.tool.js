...
const atlassian_projects_types_js_1 = require("./atlassian.projects.types.js");
const atlassian_projects_controller_js_1 = __importDefault(require("../controllers/atlassian.projects.controller.js"));
// Create a contextualized logger for this file
const toolLogger = logger_util_js_1.Logger.forContext('tools/atlassian.projects.tool.ts');
...
async function listProjects(args) {
    const methodLogger = logger_util_js_1.Logger.forContext('tools/atlassian.projects.tool.ts', 'listProjects');
    methodLogger.debug('Listing Jira projects with filters:', args);
    try {
        // Pass args directly to the controller
        const result = await atlassian_projects_controller_js_1.default.list(args);
...
async function getProject(args) {
    const methodLogger = logger_util_js_1.Logger.forContext('tools/atlassian.projects.tool.ts', 'getProject');
    methodLogger.debug(`Retrieving project details for key/ID: ${args.projectKeyOrId}`);
    try {
        // Pass args directly to the controller
        const result = await atlassian_projects_controller_js_1.default.get(args);
        methodLogger.debug('Successfully retrieved project details from controller');
        return {
            content: [
                {
                    type: 'text',
...
function registerTools(server) {
    const methodLogger = logger_util_js_1.Logger.forContext('tools/atlassian.projects.tool.ts', 'registerTools');
    methodLogger.debug('Registering Atlassian Projects tools...');
    // Register the list projects tool
    server.tool('jira_ls_projects', `Lists Jira projects accessible to the user, optionally filtering by name/key (\`name\`) or sorting (\`orderBy\`). Use this to discover available projects and find project keys/IDs needed for other tools (like \`jira_get_project\` or \`jira_list_issues\`). Supports pagination via \`limit\` and \`startAt\`. Returns a formatted list of projects including key, name, type, style, lead, and URL. Pagination information including next page instructions is included in the returned text. **Note:** Default sort is by most recently updated issue time. Requires Jira credentials to be configured.`, atlassian_projects_types_js_1.ListProjectsToolArgs.shape, listProjects);
    // Register the get project details tool
    server.tool('jira_get_project', `Retrieves comprehensive details for a specific Jira project using its key or ID (\`projectKeyOrId\`). Includes description, lead, components, versions, and other metadata. Use this after finding a project key/ID via \`jira_list_projects\` to get its full details. Returns detailed project information formatted as Markdown. Requires Jira credentials to be configured.`, atlassian_projects_types_js_1.GetProjectToolArgs.shape, getProject);
