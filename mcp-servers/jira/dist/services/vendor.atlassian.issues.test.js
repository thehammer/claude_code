...
describe('Vendor Atlassian Issues Service', () => {
    ...
    beforeAll(() => {
        ...
        if (!credentials) {
            console.warn('Skipping Atlassian Issues tests: No credentials available');
        }
    });
    ...
    describe('search', () => {
        it('should return a list of issues', async () => {
            ...
            const result = await vendor_atlassian_issues_service_js_1.default.search({
                jql: 'created >= -30d',
                maxResults: 5,
            });
        ...
        it('should handle pagination with nextPageToken', async () => {
            ...
            if (!result.isLast && result.nextPageToken) {
                const page2 = await vendor_atlassian_issues_service_js_1.default.search({
                    jql: 'created >= -30d',
                    maxResults: 2,
        ...
        it('should support searching with simple JQL (project=KEY)', async () => {
            ...
            const initialSearch = await vendor_atlassian_issues_service_js_1.default.search({
                jql: 'created >= -30d',
                maxResults: 1,
            });
        ...
        it('should support complex JQL with multiple conditions', async () => {
            ...
            try {
                ...
                if (result.issues.length >= 2) {
                    const date1 = new Date(result.issues[0].fields.created ||
                        new Date().toISOString()).getTime();
                    const date2 = new Date(result.issues[1].fields.created ||
                        new Date().toISOString()).getTime();
        ...
        it('should handle AND/OR operators in JQL', async () => {
            ...
            try {
                ...
                if (result.issues.length > 0) {
                    result.issues.forEach((issue) => {
                        const created = new Date(issue.fields.created || new Date().toISOString());
                        const sixtyDaysAgo = new Date();
                        sixtyDaysAgo.setDate(sixtyDaysAgo.getDate() - 60);
                        expect(created.getTime()).toBeGreaterThanOrEqual(sixtyDaysAgo.getTime());
                        // At least one of these should be true
                        const isBug = issue.fields.issuetype?.name === 'Bug';
                        const isHighPriority = issue.fields.priority?.name === 'High';
        ...
        it('should throw McpError for invalid JQL syntax', async () => {
            ...
            try {
                await vendor_atlassian_issues_service_js_1.default.search({
    ...
    describe('get', () => {
        ...
        async function getValidIssueKey() {
            if (skipIfNoCredentials())
                return null;
            try {
        ...
        it('should return issue details', async () => {
            ...
            const result = await vendor_atlassian_issues_service_js_1.default.get(issueKey);
            // Verify the response structure
        ...
        it('should throw McpError for non-existent issue ID', async () => {
            ...
            try {
                await vendor_atlassian_issues_service_js_1.default.get(nonExistentIssueKey);
        ...
        it('should throw McpError for invalid issue ID format', async () => {
            ...
            try {
                await vendor_atlassian_issues_service_js_1.default.get(invalidFormat);
        ...
        it('should throw McpError for empty issue ID', async () => {
            if (skipIfNoCredentials())
                return;
            try {
                await vendor_atlassian_issues_service_js_1.default.get('');
