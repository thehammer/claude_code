# queue/state.sh — shared state primitives for the queue CLI and daemon.
#
# Sourced by ~/.claude/bin/queue and ~/.claude/bin/queue-runner. Do not
# invoke directly.
#
...
: "${QUEUE_ROOT:=$HOME/.claude/queue}"
: "${JOBS_DIR:=$QUEUE_ROOT/jobs}"
: "${EVENTS_DIR:=$QUEUE_ROOT/events}"
: "${LOGS_DIR:=$QUEUE_ROOT/logs}"
: "${DRAFTS_DIR:=$QUEUE_ROOT/drafts}"
: "${CURSORS_DIR:=$QUEUE_ROOT/cursors}"
: "${RUNNER_DIR:=$QUEUE_ROOT/runner}"
...
_with_lock() {
    ...
    while ! mkdir "$lockdir" 2>/dev/null; do
        sleep 0.05
        tries=$((tries + 1))
        [ "$tries" -gt 200 ] && { echo "queue: could not acquire lock for $target after 10s" >&2; return 1; }
    done
    "$@"
    local rc=$?
...
_job_update() {
    ...
    [ -f "$_jobpath" ] || { echo "queue: no such job: $id" >&2; return 1; }
