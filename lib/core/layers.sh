...
# Sync layer lib/services/*.sh into main lib/services/ via symlinks
# Personal libs always take precedence (no symlink created if a real file exists)
layer_sync_libs() {
    local lib_dir="$CLAUDE_HOME/lib/services"
    local bash_dir="$CLAUDE_HOME/lib/services/_bash"
    mkdir -p "$lib_dir" "$bash_dir"
    ...
    while IFS= read -r layer; do
        ...
        # Sync lib/services/*.sh (top-level service libs)
        if [[ -d "$LAYERS_DIR/$layer/lib/services" ]]; then
            for file in "$LAYERS_DIR/$layer/lib/services"/*.sh; do
                [[ -f "$file" ]] || continue
                basename=$(basename "$file")

                # Skip if personal lib exists (real file, not symlink)
                if [[ -f "$lib_dir/$basename" && ! -L "$lib_dir/$basename" ]]; then
                    continue
                fi

                target="$lib_dir/$basename"
                [[ -L "$target" ]] && rm "$target"
                ln -s "$file" "$target"
                synced_files+=("$basename")
                ((synced++))
        ...
        # Sync lib/services/_bash/*.sh (CLI wrapper libs)
        if [[ -d "$LAYERS_DIR/$layer/lib/services/_bash" ]]; then
            for file in "$LAYERS_DIR/$layer/lib/services/_bash"/*.sh; do
                [[ -f "$file" ]] || continue
                basename=$(basename "$file")

                if [[ -f "$bash_dir/$basename" && ! -L "$bash_dir/$basename" ]]; then
                    continue
                fi

                target="$bash_dir/$basename"
                [[ -L "$target" ]] && rm "$target"
                ln -s "$file" "$target"
                synced_files+=("_bash/$basename")
                ((synced++))
            done
        fi
    done < <(layer_list)

    # Write .gitignore for synced lib symlinks
    if [[ ${#synced_files[@]} -gt 0 ]]; then
        {
            echo "# Auto-generated by layer_sync_libs — do not edit"
            echo "# These are symlinks to layer service libraries"
            printf '%s\n' "${synced_files[@]}"
        } > "$lib_dir/.gitignore"
    fi
...
# Full sync: commands, hooks, agents, skills, libs, bin/services, cleanup
layer_sync() {
    ...
    local lib_count=$(layer_sync_libs)
    ...
    local lib_cleaned=$(layer_cleanup_symlinks "$CLAUDE_HOME/lib/services")
    local bash_cleaned=$(layer_cleanup_symlinks "$CLAUDE_HOME/lib/services/_bash")
    local svc_cleaned=$(layer_cleanup_symlinks "$CLAUDE_HOME/bin/services")

    echo "Synced: $cmd_count commands, $hook_count hooks, $agent_count agents, $skill_count skills, $lib_count libs, $svc_count services"
    local total_cleaned=$((cmd_cleaned + hook_cleaned + agent_cleaned + skill_cleaned + lib_cleaned + bash_cleaned + svc_cleaned))
    if [[ $total_cleaned -gt 0 ]]; then
        echo "Cleaned: $total_cleaned stale symlinks"
    fi
    return 0
}
